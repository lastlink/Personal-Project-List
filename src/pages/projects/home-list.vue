<template>
  <f7-page>
    <f7-navbar title="Home List" back-link="Back">
    ...
    <f7-block>
      <p>{{List.home_list.description}}</p>
    </f7-block>
...
<script>
  ...
  export default {
    ...
    data() {
      const items = [];
      for (let i = 0; i < List.home_list.list.length; i++) {
        items.push(List.home_list.list[i])
        // items.push({
        //   title: `${List.home_list.list[i].title}`,
        //   subtitle: `${List.home_list.list[i].subtitle}`,
        //   link: List.home_list.list[i].url
        // });
      }

      console.log("LOGGING LIST:...")
      console.log(List)
      // const home_list_list=List.home_list.list
      // console.log(home_list_list)
      return {
        List,
        items,
        vlData: {
